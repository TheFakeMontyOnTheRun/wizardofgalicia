...
namespace WizardOfGalicia {
	...
	bool CMap::cast(std::shared_ptr<CActor> actor) {
		...
		auto fireball = std::make_shared<CFireball>(actor->direction, energyInvestment,
		                                            actor->position);
		...
		for (int y = 0; y < 20; ++y) {
			for (int x = 0; x < 20; ++x) {
				...
				switch (element) {
					...
					case '9':
					case '*':
						mDoorways.push_back(std::make_shared<CDoorway>(Vec2i(x, y), element == '9'
						                                                            ? DoorwayFunction::Exit
						                                                            : DoorwayFunction::Entry));
						break;
					case '5':
	...
	bool CMap::isAtExit(std::shared_ptr<CActor> character) {
		for (auto doorway : mDoorways) {
			if (doorway->doorFunction == DoorwayFunction::Exit &&
			    character->position == doorway->position) {
				return true;
			}
		}
	...
	bool CMap::isAtEntrance(std::shared_ptr<CActor> character) {
		for (auto doorway : mDoorways) {
			if (doorway->doorFunction == DoorwayFunction::Entry &&
			    character->position == doorway->position) {
				return true;
			}
		}
